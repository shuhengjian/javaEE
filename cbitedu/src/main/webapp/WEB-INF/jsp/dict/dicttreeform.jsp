...
<script type="text/javascript">
	...
	function save_click() {
		...
		if (bool == true) {
			$.ajax({
				url : "${ctx}/tsysDictController.do?method=ajaxSaveDataTree",
				type : "POST",
				data : $("#tsysDictForm").serialize(),
				dataType : 'json',
				error : function() {
					jQuery.messager.alert('提示:', '发生错误!', 'info');
				},
...
<cbitedu:form bean="tsysDict" scope="request">
	...
	<form action="${ctx}/tsysDictController.do?method=ajaxSave"
		...
		<input type="hidden" id="dictId" name="dictId" /> <input
			type="hidden" id="displaySort" name="displaySort" /> <input
			type="hidden" id="parentType" name="parentType" />
		<!-- 取得父类的id -->
		<div>
			<table class="FormView" border="0" cellspacing="1" cellpadding="0">
				...
				<tr>
					<td><fmt:message key="tsysdict.dictValue" />
						<!-- 字典值 --></td>
					<td><input type="text" id="dictValue" name="dictValue"
						class="text validate[required,maxSize[10]]" /> <font color="red">*</font>
					</td>
				</tr>
				<tr>
					<td><fmt:message key="tsysdict.dictCode" />
						<!-- 字典编码 --></td>
				...
				<tr>
					<td><fmt:message key="tsysdict.sortNum" />
						<!-- 排序号 --></td>
					<td><input type="text" id="sortNum" name="sortNum"
						class="text validate[maxSize[10]]" /></td>
				</tr>
				<tr>
					<td>默认值 <!-- 字典描述 --></td>
				...
				<%-- 			<tr>
				<td>
					<fmt:message key="tsysdict.multiType" /><!-- 单选或多选 -->
				</td>
				<td>
					<cbitedu:para id="multiType" name="dictMultiType" cssClass="select" withEmpty="true"/>
				</td>
			</tr> --%>
				<tr>
					<td><fmt:message key="tsysdict.orgId" />
						<!-- 所属机构 --></td>
					<td><input type="text" id="orgId" name="orgId"
						class="text validate[maxSize[10]]" /></td>
				</tr>
			</table>
		</div>
