package com.creatorblue.cbitedu.core.plugins.poi.excel.export.base;
...
import com.creatorblue.cbitedu.core.plugins.poi.excel.entity.params.MergeEntity;
...
public abstract class ExcelExportBase extends ExportBase {
	...
	public int createCells(Drawing patriarch, int index, Object t,
		...
		for (int k = 0, paramSize = excelParams.size(); k < paramSize; k++) {
			...
			if (entity.getList() != null) {
				...
				if(entity.getCode().equals("rownum_")){
					int value=index-1;//序号
					createNumberCell(
							row,
							cellNum++,
							String.valueOf(value),
							styles, entity,workbook);
				}else if(!entity.getDataformat().equals("-1")){
	 ...
	 * @param entity
	 */
	public void createNumberCell(Row row, int index, String text,
			Map<String, HSSFCellStyle> styles, ExcelExportEntity entity,Workbook wb) {
		Cell cell = row.createCell(index);
		CellStyle style=getCellstyle(styles,entity);
		
		if(text!=null && !text.equals("")){
			HSSFDataFormat df = (HSSFDataFormat) wb.createDataFormat();
			style.setDataFormat(df.getFormat(entity.getDataformat()));
  ...
  public void createStringCell(Row row, int index, String text,
    ...
    //style.setAlignment(getAligntype(entity));
    cell.setCellValue(Rtext);
    if (style != null) {
      cell.setCellStyle(style);
    }
  }
  public void createStringCell_2(Row row, int index, String text,
		  Map<String, HSSFCellStyle> styles, ExcelExportEntity entity,Workbook wb) {
    RichTextString Rtext = new HSSFRichTextString(text);
    Cell cell = row.createCell(index);
	CellStyle style=getCellstyle(styles,entity);
	 ...
	 * @return
	 */
	public CellStyle getCellstyle(Map<String, HSSFCellStyle> styles,ExcelExportEntity entity) {
		CellStyle style=null;
		if(entity.getAlign().equals("left")){
			if(entity.isWrap())
				style=styles.get("oneWrap");
			else
				style=styles.get("one");;
		}else{
			if(entity.getAlign().equals("right")){
				if(entity.isWrap())
					style=styles.get("threeWrap");
				else
					style=styles.get("three");;
			}else{
				if(entity.isWrap())
					style=styles.get("twoWrap");
				else
