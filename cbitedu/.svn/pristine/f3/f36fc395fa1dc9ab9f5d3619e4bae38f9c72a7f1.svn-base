...
<script type="text/javascript">
	$(function() {
		$("#tsys_manager_list").flexigrid({
			url : "${ctx}/ttyBackPriceController.do?method=query",// ajax方式对应的url地址
			type : 'post',// 数据发送方式 
			dataType : 'json',// 数据加载的类型(xml/json)        
			contentType : 'application/x-www-form-urlencoded; charset=UTF-8',
			sortname : "price_id",//排序
			sortorder : "asc",
			useRp : true,
			checkbox : true,
			//radio : true,// 是否要多选框
			idProperty : 'priceId',// 多选框绑定行的id
	...
	$("#btn_edit").click(function() {
			...
			dialogFun = sy.modalDialog({
				...
				height : 300,
				resizable : true,
				url : '${ctx}/ttyBackPriceController.do?method=modify&id='+id
			});
		});
				...
				function() {
					...
					jQuery.messager.confirm('提示:', '你确认要删除吗?', function(event) {
						if (event) {
							...
							$.ajax({
								url : "${ctx}/ttyBackPriceController.do",
								data : {
									method : "del",
									priceIds : priceIds
								},
	...
	var btn_add = function() {
		dialogFun = sy.modalDialog({
			...
			height : 300,
			resizable : true,
			url : '${ctx}/ttyBackPriceController.do?method=add'
		});
	};
	// 关闭窗口
		...
		<div region="north" border="false">
			<div id="query_area">
				...
				<table class="FormView" border="0" cellspacing="1" cellpadding="0">
					...
					<tr>
						<td align="left"><!-- 价格最小值 -->
							<fmt:message key="price.priceMin" />
						</td>
						<td>
							<input type="text" name="price_priceMin" id="priceMin" class="text">
						</td>
						<td align="left"><!-- 价格最大值 -->
							<fmt:message key="price.priceMax" />
						</td>
						<td>
							<input type="text" name="price_priceMax" id="priceMax" class="text">
						</td>
						<td align="left"><!-- 价格区间状态 -->
							<fmt:message key="price.priceState" />
						</td>
						<td>
							<cbitedu:para name="brand_state" id="priceState" cssClass="select" withEmpty="true" />
						</td>
					</tr>
				</table>
			</div>
