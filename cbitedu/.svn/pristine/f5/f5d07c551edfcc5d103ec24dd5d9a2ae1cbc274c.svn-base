...
public class DateUtils {
	...
	public static void main(String[] args) {
		log.info("字符串转成日期1" + DateUtils.StrToDateFormat("yyyy-MM-dd","2001-02-17"));
		log.info("字符串转成日期2" + DateUtils.StrToDateFormat("yyyyMMdd","20010217"));
	}


	// Date d = DateFormat("yyyy-MM-dd","2001-02-17");
	// Date d = DateFormat("yyyyMMdd","20010217");
	public static Date StrToDateFormat(String format, String datetime) {
		SimpleDateFormat sdf = null;
		Date d = null;

		try {
			sdf = new SimpleDateFormat(format, Locale.CHINA);
			d = (java.util.Date) sdf.parse(datetime);
		} catch (Exception e) {
			log.info(e);
		}
	...
	public static Collection<Object> getAllWeekForDay(String yy, String mm,
		...
		long lMax = StrToDate(max).getTime();
		...
		if ((incol != null) && (incol.size() > 0)) {
			...
			while (weekit.hasNext()) {
				...
				java.util.Date date = StrToDate(strDay);
