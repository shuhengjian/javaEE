 ...
 * @param srcData
 */
function convertData(srcData,menu_){
	var returnData=[];
	var topData = $.grep(srcData,function(o,i){return o.parentModuleid==menu_.menuid;});
	$(topData).each(function(){
		var menu={};//顶级菜单
		menu.menuid=this.moduleId;
		menu.icon=this.icon;
		menu.menuname=this.moduleName;
		menu.url=this.linkAddr;
		menu.child=convertData(srcData,menu);
...
function loadTreeMenu(){
	$("#menuTree").show();
	$("#nav").hide();
	if($("#menuTree").html().length>0){
		return;
	}
	var zNodes=menuData;
	$(zNodes).each(function(){
		this.iconSkin=this.icon;
		this.icon_easyUI=this.icon;
		this.icon=undefined;
	});
			...
			callback: {
				onClick:function(e, id, node){
					...
					if(node.isParent) {
						...
						addTab(node.moduleName, node.linkAddr,"icon "+node.icon_easyUI);
					}
				}
			}
	};
...
//改变菜单树的风格
function changeMenuStyle(mstyle){
	if(mstyle.src.indexOf('tree')>-1){
		loadTreeMenu();
	}else{
		loadListMenu();
	}
