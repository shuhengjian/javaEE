...
sy.modalDialog = function(options) {
	...
	var dv=$('<div/>');
	if (options.url) {//tbw 使用url属性时会创建一个iframe展示链接页面内容，解决弹出层被object或select遮住的问题。但是iframe中的某些浮动层将无法显示到iframe以外的地方
		var id="dialogIframe"+new Date().getTime();
		var iframeObj=$('<iframe id="'+id+'" src="' + options.url + '" allowTransparency="true" scrolling="auto" width="100%" height="100%" frameBorder="0" name=""></iframe>');
		if(options.onLoad){//给iframe添加监听事件使onLoad方法有效
			var oFrm = iframeObj.get(0);
			var fun_onLoad= function() {
			     if (this.readyState && this.readyState != 'complete') return;
			     else {
			    	 options.onLoad();
			     }
			}
			if(navigator.appName !=  "Microsoft Internet Explorer"){//alert("onload");
				oFrm.onload = fun_onLoad;
			}else{//alert("onreadystatechange");
				oFrm.onreadystatechange = fun_onLoad;
			}
		}
		dv.append(iframeObj);
	}
	
	return dv.dialog(opts);
};
...
sy.changeTheme = function(themeName) {
	...
	var $iframe = $('iframe');
	var changeIfrTheme=function($iframe){
		if ($iframe.length > 0) {
			for (var i = 0; i < $iframe.length; i++) {
				...
				try {
					...
					trueIfr=$(ifr).contents();
				} catch (e) {
					try {
						ifr.contentWindow.document.getElementById('easyuiTheme').href = href;
						trueIfr=$(ifr).contentWindow.document;
					} catch (e) {
					}
				}
				changeIfrTheme($('iframe',trueIfr));//tbw 递归调用更换皮肤的方法，把窗口中所有深度的iframe都更换一次
			}
		}
	};
	changeIfrTheme($iframe);
