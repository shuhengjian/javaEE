...
(function ($) {
	...
	$.addFlex = function (t, p) {
		...
		p = $.extend({ //apply default properties
			...
			onSuccess: false,
			onError: false,
			onRowSelect:false,//function(selRows)
		...
		var g = {
			...
			getCheckCol:function(){
				var cb = $(':checkbox:checked', this.nDiv);
				var returnRes=new Array();
				$(cb).each(function(){
					var r={};
					r.value=$(this).val();
					r.display=$(this).closest("tr").find("td[class=ndcol2]").html();
					r.name=p.colModel[r.value].name;
					returnRes.push(r);
				});
				return returnRes;
			},
						...
						$('thead tr:first th', g.hDiv).each( //add cell
							function () {
								...
								if (typeof row.cell == 'undefined') {
									var rname=p.colModel[idx].name;
									var rdata;
									if(rname.indexOf(".")>0){//tbw 让colModel的name属性支持javabean规范的数据，比如a.b.c这样的属性，同时把列的默认值改成空字符串
										rname=rname.split(".");
										rdata=row;
										$(rname).each(function(){
											rdata=rdata[this]?rdata[this]:"";
										});
									}else{
										rdata=row[rname]?row[rname]:"";
									}
									td.innerHTML = rdata;
								} else {
...
//						    cthch.addClass("noborder").click(function() {//add matychen 不要noborder，加入后ie8显示有问题。
			...
			populate: function () { //get latest data
				...
				$('.pPageStat', this.pDiv).html(p.proitemg);
			...
			addCellProp: function () {
				$('tbody tr td', g.bDiv).each(function (i) {
					...
					if (pth != null) {
						//------------------dwq------------------------------
						if(g.data && g.data.rows && g.data.rows.length != 0) {
							if (pth.process) pth.process(tdDiv, pid, g.data.rows[parseInt((i / p.colModel.length), 10)]);
						} else {
...
//						if (pth.process) pth.process(tdDiv, pid);
	...
	$.fn.getCheckCol = function () { //得到选中的列，有value列索引，display显示名称和name属性名 tbw
		var res=new Array();
		this.each(function () {
			if (this.grid) res=res.concat(this.grid.getCheckCol());
		});
		return res;
	};
