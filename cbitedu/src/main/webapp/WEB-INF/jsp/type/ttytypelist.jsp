...
<script type="text/javascript">
	$(function() {
		$("#tsys_manager_list").flexigrid({
			url : "${ctx}/ttyBackTypeController.do?method=query",// ajax方式对应的url地址
			type : 'post',// 数据发送方式 
	...
	$("#btn_edit").click(function() {
			...
			dialogFun = sy.modalDialog({
				...
				height : 300,
				resizable : true,
				url : '${ctx}/ttyBackTypeController.do?method=modify&id='+id
			});
		});
		...
		function() {
			...
			jQuery.messager.confirm('提示:', '你确认要删除吗?', function(event) {
				if (event) {
					...
					$.ajax({
						url : "${ctx}/ttyBackTypeController.do",
						data : {
							method : "del",
							typeIds : typeIds
						},
	...
	var btn_add = function() {
		dialogFun = sy.modalDialog({
			...
			height : 300,
			resizable : true,
			url : '${ctx}/ttyBackTypeController.do?method=add'
		});
	};
	// 关闭窗口
		...
		<div region="north" border="false">
			<div id="query_area">
				...
				<table class="FormView" border="0" cellspacing="1" cellpadding="0">
					...
					<tr>
						<td align="left"><!-- 类型名称 -->
							<fmt:message key="type.typeName" />
						</td>
						<td>
							<input type="text" name="type_typeName" id="typeName" class="text" value="">
						</td>
						<td align="left"><!-- 类型状态 -->
							<fmt:message key="type.typeState" />
						</td>
						<td>
							<cbitedu:para name="brand_state" id="typeState" cssClass="select" withEmpty="true" />
						</td>
					</tr>
				</table>
			</div>
