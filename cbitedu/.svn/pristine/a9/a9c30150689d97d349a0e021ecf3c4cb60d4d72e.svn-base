...
<script type="text/javascript">
	$(function() {
		...
		$("#tsys_manager_list").flexigrid({
			url : "${ctx}/typeController.do?method=query",// ajax方式对应的url地址
			type : 'post',// 数据发送方式 
			dataType : 'json',// 数据加载的类型(xml/json)        
			contentType : 'application/x-www-form-urlencoded; charset=UTF-8',
			sortname : "type_name",//排序
			sortorder : "asc",
			useRp : true,
			radio : true,// 是否要多选框
		...
		$("#btn_edit").click(function() {
			...
			dialogFun = sy.modalDialog({
				...
				height : 300,
				resizable : true,
				url : '${ctx}/typeController.do?method=modify&id='+id
			});
		});
	...
	var btn_add = function() {
		dialogFun = sy.modalDialog({
			...
			height : 300,
			resizable : true,
			url : '${ctx}/typeController.do?method=add'
		});
	};
			...
			<div id="query_area">
				...
				<table class="FormView" border="0" cellspacing="1" cellpadding="0">
					...
					<tr>
						...
						<td>
							<input type="text" name="typeName" id="postName" class="text" value="">
						</td>
					</tr>
				</table>
			</div>
