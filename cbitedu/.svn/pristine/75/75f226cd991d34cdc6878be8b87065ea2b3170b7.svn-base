...
<script type="text/javascript">
	$(function() {
		...
		$("#tsys_manager_list").flexigrid({
			url : "${ctx}/tsysPostController.do?method=query",// ajax方式对应的url地址
			type : 'post',// 数据发送方式 
		...
		$("#btn_edit").click(function() {
			...
			dialogFun = sy.modalDialog({
				...
				height : 300,
				resizable : true,
				url : '${ctx}/tsysPostController.do?method=modify&id='+id
			});
		});
	...
	var btn_add = function() {
		dialogFun = sy.modalDialog({
			...
			height : 300,
			resizable : true,
			url : '${ctx}/tsysPostController.do?method=add'
		});
	};
		...
		<div region="north" border="false">
			<div id="query_area">
				...
				<table class="FormView" border="0" cellspacing="1" cellpadding="0">
					...
					<tr>
						<td align="left"><!-- 岗位名称 -->
							<fmt:message key="tsyspost.postName" />
						</td>
						<td>
							<input type="text" name="post_postName" id="postName" class="text" value="">
						</td>
						<td align="left"><!-- 岗位类型 -->
							<fmt:message key="tsyspost.postType" />
						</td>
						<td>
							<cbitedu:para name="postType" id="postType" cssClass="select" withEmpty="true" />
						</td>
					</tr>
				</table>
			</div>
